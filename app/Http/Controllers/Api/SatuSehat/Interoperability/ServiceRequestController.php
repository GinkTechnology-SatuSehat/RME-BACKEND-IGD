...
class ServiceRequestController extends Controller
{
        ...
        if (isset($data_service_request['reasonCode'])) {
            for ($i = 0; $i < count($data_service_request['reasonCode']); $i++) {
                $this->service_request['reasonCode'][] = [
                    'coding' => [
                        [
                            "system" => $data_service_request['reasonCode'][$i]['system'],
                            'code' => $data_service_request['reasonCode'][$i]['code'],
                        ]
                    ]
                ];

                if (isset($data_service_request['reasonCode'][$i]['display'])) {
                    $this->service_request['reasonCode'][$i]['coding'][0]['display'] = $data_service_request['reasonCode'][$i]['display'];
                }

                if (isset($data_service_request['reasonCode'][$i]['text'])) {
                    $this->service_request['reasonCode'][$i]['text'] = $data_service_request['reasonCode'][$i]['text'];
                }
            }
        }
    }
    ...
    public function add_locationCode($data_service_request)
    {
        if (isset($data_service_request['locationCode'])) {
            for ($i = 0; $i < count($data_service_request['locationCode']); $i++) {
                $this->service_request['locationCode'][] = [
                    'coding' => [
                        [
                            "system" => "http://terminology.hl7.org/CodeSystem/v3-RoleCode",
                            'code' => $data_service_request['locationCode'][$i]['code'],
                        ]
                    ]
                ];

                if (isset($data_service_request['locationCode'][$i]['display'])) {
                    $this->service_request['locationCode'][$i]['coding'][0]['display'] = $data_service_request['locationCode'][$i]['display'];
                }
            }
        }
    }
    ...
    public function json($data_service_request)
    {
        ...
        $this->add_locationCode($data_service_request);
