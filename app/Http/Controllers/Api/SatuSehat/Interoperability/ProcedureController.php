...
class ProcedureController extends Controller
{
    ...
    public function add_bodySite($data_procedure)
    {
        if (isset($data_procedure['bodySite'])) {
            for ($i = 0; $i < count($data_procedure['bodySite']); $i++) {
                $this->procedure['bodySite'][] = [
                    'coding' => [
                        [
                            'system' => "http://snomed.info/sct",
                            'code' => $data_procedure['bodySite'][$i]['code'],
                        ]
                    ]
                ];

                if (isset($data_procedure['bodySite'][$i]['display'])) {
                    $this->procedure['bodySite'][$i]['coding'][0]['display'] = $data_procedure['bodySite'][$i]['display'];
                }
            }
        }
    }
    ...
    public function add_reasonCode($data_procedure)
    {
        if (isset($data_procedure['reasonCode'])) {
            for ($i = 0; $i < count($data_procedure['reasonCode']); $i++) {
                $this->procedure['reasonCode'][] = [
                    'coding' => [
                        [
                            'system' => $data_procedure['reasonCode'][$i]['system'],
                            'code' => $data_procedure['reasonCode'][$i]['code'],
                        ]
                    ]
                ];

                if (isset($data_procedure['reasonCode'][$i]['display'])) {
                    $this->procedure['reasonCode'][$i]['coding'][0]['display'] = $data_procedure['reasonCode'][$i]['display'];
                }
            }
        }
    }
