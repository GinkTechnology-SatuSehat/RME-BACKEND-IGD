...
class ConditionController extends Controller
{
    ...
    public function add_note($data_condition)
    {
        if (isset($data_condition['note'])) {
            for ($index = 0; $index < count($data_condition['note']); $index++) {
                if (isset($data_condition['note'][$index]['text'])) {
                    $this->condition['note'][] = [
                        'text' => $data_condition['note'][$index]['text']
                    ];
                }
            }
        }
    ...
    public function add_verificationStatus($data_condition)
    {
        if (isset($data_condition['verificationStatus'])) {
            $this->condition['verificationStatus'] = [
                'coding' => [
                    [
                        'system' => 'http://terminology.hl7.org/CodeSystem/condition-ver-status',
                        'code' => $data_condition['verificationStatus']['code'],
                    ]
                ]
            ];

            if (isset($data_condition['verificationStatus']['display'])) {
                $this->condition['verificationStatus']['coding'][0]['display'] = $data_condition['verificationStatus']['display'];
            }
        }
    }
    ...
    public function json($data_condition)
    {
        ...
        $this->add_note($data_condition);
        $this->add_verificationStatus($data_condition);
